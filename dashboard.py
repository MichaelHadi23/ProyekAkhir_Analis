import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import streamlit as st
from babel.numbers import format_currency  # Pastikan modul format_currency diimpor

sns.set(style='dark')

# Definisi fungsi untuk membuat DataFrame harian dari main_df
def create_daily_orders_df(df):
    df['dteday'] = pd.to_datetime(df['dteday'])
    orders_df = df.resample('D', on='dteday').sum()
    return orders_df

# Definisi fungsi untuk membuat DataFrame casual rental berdasarkan hari
def create_ren_cas_df(df):
    ren_cas_df = df.groupby("weekday").casual.sum().sort_values(ascending=False).reset_index()
    return ren_cas_df

# Definisi fungsi untuk membuat DataFrame registered rental berdasarkan hari
def create_ren_reg_df(df):
    ren_reg_df = df.groupby("weekday").registered.sum().sort_values(ascending=False).reset_index()
    return ren_reg_df

# Definisi fungsi untuk membuat DataFrame berdasarkan bulan (bymonth_df)
def create_bymonth_df(df):
    bymonth_df = df.groupby("month").Total.sum().reset_index()
    return bymonth_df

# Definisi fungsi untuk membuat DataFrame berdasarkan cuaca (byweather_df)
def create_byweather_df(df):
    # Implementasi logika untuk create_byweather_df
    # Misalnya:
    byweather_df = df.groupby("weather_situation").Total.sum().reset_index()
    return byweather_df

# Definisi fungsi untuk membuat DataFrame berdasarkan RFM parameters (rfm_df)
def create_rfm(df):
    rfm_df = df.groupby(by="hour", as_index=False).agg({
        "dteday": "max", 
        "instant": "nunique", 
        "Total": "sum" 
    })
    rfm_df.columns = ["hour", "max_order_timestamp", "frequency", "monetary"]
    rfm_df["max_order_timestamp"] = rfm_df["max_order_timestamp"].dt.date
    recent_date = df["dteday"].dt.date.max()
    rfm_df["recency"] = rfm_df["max_order_timestamp"].apply(lambda x: (recent_date - x).days)
    rfm_df.drop("max_order_timestamp", axis=1, inplace=True)
    return rfm_df

# Memuat data dari file CSV
hour_df = pd.read_csv("hour.csv")

# Konversi kolom datetime ke tipe datetime
datetime_columns = ["dteday"]
for column in datetime_columns:
    hour_df[column] = pd.to_datetime(hour_df[column])

# Menentukan tanggal minimum dan maksimum
min_date = hour_df["dteday"].min()
max_date = hour_df["dteday"].max()

# Membuat sidebar di aplikasi Streamlit
with st.sidebar:
    # Menambahkan logo perusahaan
    st.image("bike.jpg")
    
    # Input rentang waktu dengan date_input
    start_date, end_date = st.date_input(
        label='Rentang Waktu',
        min_value=min_date,
        max_value=max_date,
        value=[min_date, max_date]
    )

# Filtering data berdasarkan rentang waktu yang dipilih
main_df = hour_df[(hour_df["dteday"] >= str(start_date)) & (hour_df["dteday"] <= str(end_date))]

# Memanggil fungsi untuk membuat DataFrames yang diperlukan
daily_orders_df = create_daily_orders_df(main_df)
ren_cas_df = create_ren_cas_df(main_df)
ren_reg_df = create_ren_reg_df(main_df)
bymonth_df = create_bymonth_df(main_df)
byweather_df = create_byweather_df(main_df)
rfm_df = create_rfm(main_df)

# Tampilkan header dan subheader di aplikasi Streamlit
st.header('Bike Sharing Dashboard :sparkles:')
st.subheader('Daily Rental')

# Menampilkan metrik untuk total casual, registered, dan total rental
col1, col2, col3 = st.columns(3)
with col1:
    total_casual = daily_orders_df.casual.sum()
    st.metric("Total Casual Rental", value=f'{total_casual:,}')

with col2:
    total_registered = daily_orders_df.registered.sum()
    st.metric("Total Registered Rental", value=f'{total_registered:,}')
 
with col3:
    total_revenue = daily_orders_df.Total.sum()
    st.metric("Total Rental", value=f'{total_revenue:,}')

# Menampilkan plot jumlah rental dari waktu ke waktu
plt.figure(figsize=(10, 6))
plt.plot(daily_orders_df.index, daily_orders_df['Total'], color='#A5C0DD')
plt.xlabel(None)
plt.ylabel(None)
plt.title('Number of Rental Over Time')
plt.xticks(rotation=45)
plt.grid(True)
plt.tight_layout()
st.pyplot(plt)

# Menampilkan performa casual dan registered rental berdasarkan hari
st.subheader("Performing Casual and Registered Rental by Day")
fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(35, 15))
colors = ["#72BCD4", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3"]

# Plot untuk casual rental
sns.barplot(x="casual", y="weekday", data=ren_cas_df, palette=colors, ax=ax[0])
ax[0].set_ylabel(None)
ax[0].set_xlabel("Number of Casual", fontsize=30)
ax[0].set_title("Performing Casual Rental by Day", loc="center", fontsize=50)
ax[0].tick_params(axis='y', labelsize=35)
ax[0].tick_params(axis='x', labelsize=30)

# Plot untuk registered rental
sns.barplot(x="registered", y="weekday", data=ren_reg_df, palette=colors, ax=ax[1])
ax[1].set_ylabel(None)
ax[1].set_xlabel("Number of Registered", fontsize=30)
ax[1].invert_xaxis()
ax[1].yaxis.set_label_position("right")
ax[1].yaxis.tick_right()
ax[1].set_title("Performing Registered Rental by Day", loc="center", fontsize=50)
ax[1].tick_params(axis='y', labelsize=35)
ax[1].tick_params(axis='x', labelsize=30)

st.pyplot(fig)

# Menampilkan jumlah rental berdasarkan bulan dan cuaca
st.subheader("Number of Rental")
col1, col2 = st.columns(2)

# Plot untuk rental berdasarkan bulan
with col1:
    fig, ax = plt.subplots(figsize=(35, 15))
    sns.barplot(
        y="Total", 
        x="month",
        data=bymonth_df.sort_values(by="month", ascending=False),
        palette=colors,
        ax=ax
    )
    ax.set_title("Number of Rental by Month", loc="center", fontsize=50)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
    ax.tick_params(axis='x', labelsize=35, rotation=45)
    ax.tick_params(axis='y', labelsize=30)
    st.pyplot(fig)

# Plot untuk rental berdasarkan cuaca
with col2:
    fig, ax = plt.subplots(figsize=(35, 15))
    sns.barplot(
        y="Total", 
        x="weather_situation",
        data=byweather_df.sort_values(by="weather_situation", ascending=False),
        palette=colors,
        ax=ax
    )
    ax.set_title("Number of Rental by Weather Situation", loc="center", fontsize=50)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
    ax.tick_params(axis='x', labelsize=35)
    ax.tick_params(axis='y', labelsize=30)
    st.pyplot(fig)

# Menampilkan performa rental berdasarkan parameter
